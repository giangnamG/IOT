...
export default function DataStreamLogsComponent({ props }) {
    ...
    const [filterDust, setFilterDust] = useState('');
    const [filterRain, setFilterRain] = useState('');
    ...
    }, [currentPage, perPage, isRealTime, isLatest, fromDay, toDay, filterTemp, filterHumidity, filterLight, filterTimestamp, filterDust, filterRain, filterWind]);
    ...
    const filteredAndSortedData = useMemo(() => {
        ...
        if (filterDust) {
            filteredItems = filteredItems.filter(item =>
                item.dust.toString().includes(filterDust)
            );
        }
        if (filterRain) {
            filteredItems = filteredItems.filter(item =>
                item.dust.toString().includes(filterRain)
            );
        }
        if (filterWind) {
            filteredItems = filteredItems.filter(item =>
                item.dust.toString().includes(filterWind)
            );
        }
    ...
    }, [data.data, filterTemp, filterHumidity, filterLight, filterDust, filterRain, filterWind, filterTimestamp, sortConfig.key, sortConfig.direction]);
    ...
    return (
        <Container >
            ...
            <div style={{
                ...
                <div style={{ maxHeight: 965, overflowY: 'auto', width: 1700 }}>
                            ...
                            <tr>
                                ...
                                <th style={{ ...styles.table, fontSize: '18px' }}>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setFilterDust(e.target.value)}
                                    />
                                </th>
                                <th style={{ ...styles.table, fontSize: '18px' }}>
                                    Mức Gió
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setFilterWind(e.target.value)}
                                    />
                                </th>
                                <th style={{ ...styles.table, fontSize: '18px' }}>
                                    Mức Mưa
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setFilterRain(e.target.value)}
                                    />
                                </th>
                                <th style={{ ...styles.table, fontSize: '18px' }}>
                                    Thời Gian
                            ...
                            {Array.isArray(filteredAndSortedData) && filteredAndSortedData.length > 0 &&
                                filteredAndSortedData.map((item, index) => (
                                    <tr key={index} style={{ cursor: 'pointer' }}>
                                        ...
                                        <td style={{ ...styles.table }}>{item.dust}</td>
                                        <td style={{ ...styles.table }}>{item.wind}</td>
                                        <td style={{ ...styles.table }}>{item.rain}</td>
